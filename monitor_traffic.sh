...
max_percentage=${2:-90}  # 如果用户未输入，默认值为 90%

reset_day=${3:-1}

CHECK_TYPE=${4:-4}

ssh_port=${5:-22}  # 如果用户未输入，默认值为 22

INTERFACE=${6:-$(ip route | grep default | awk '{print $5}')}

# 计算流量限制值，单位为 MiB，并应用最大百分比
LIMIT=$(echo "$LIMIT_GB * 1024 * $max_percentage / 100" | bc)
...
if (( $(echo "$TRAFFIC_TO_CHECK > $LIMIT" | bc -l) )); then
  ...
  echo "警告：流量已超出限制！除SSH（端口$ssh_port）外，所有端口已被阻止。"
